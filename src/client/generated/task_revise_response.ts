...
export type TaskRunTurnKind = 'initial' | 'revision' | 'recovery' | 'review';
export type TaskRunTurnStatus =
  'in_progress' | 'candidate_created' | 'review_recorded' | 'failed' | 'blocked' | 'interrupted' | 'cancelled';

export interface TaskReviseResponse {
...
export interface TaskRunTurn {
  ...
  requestedByReviewEventId?: string | null;
  reviewsCandidateId?: string | null;
  round: number;
  runId: string;
  sequence: number;
  startedAt?: number | null;
