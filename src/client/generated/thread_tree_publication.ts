...
export interface ThreadTreePublication {
  actions?: {
    [k: string]: DirectoryActionPublication;
  };
...
export interface DirectoryActionPublication {
  error?: string | null;
  generation: number;
  pending: boolean;
  revision: number;
...
export interface SidebarChangeSet {
  changed: SidebarNodeId[];
  removed: SidebarNodeId[];
  reordered_folders: string[];
  [k: string]: unknown;
}
